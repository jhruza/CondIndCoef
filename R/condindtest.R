...
cond.ind.coef <- function(X,Y,Z=NULL, plot=FALSE, method='cdf', speedupmatrix=NULL) {
  X<-as.matrix(X)
  Y<-as.matrix(Y)
  n<-dim(X)[1]
  ...
  if (!is.null(Z)) {
    ...
    #################### Visualization: plotting the two CDFs if Z=NULL
    if (plot) {
      cdf.1<-vector()
      cdf.2<-vector()
      for (j in 1:n) {
        idx<-multi.which(X,X[j,])
        cdf.1[length(cdf.1)+1]<-length(idx)/n  
        cdf.2[length(cdf.2)+1]<-sum(w.Z.Y[j,idx])
        
      }
      print(ggplot()+geom_point(aes(x=X, y = cdf.1, color='cdf X'))+geom_point(aes(x=X, y = cdf.2, color='cdf X =x |Y=y')))
    }
    ##################### 
  }
